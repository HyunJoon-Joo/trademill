...
export class MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.addUi(
            this.add.text(640, 300, 'UPDATING MARKET MAP...', {
                fontFamily: 'Arial',
                fontSize: '24px',
                color: '#cbd5e1'
            }).setOrigin(0.5)
    ...
    async refreshMapIndexAndShowMenu() {
        try {
            const response = await fetch(
                getDataUrl(MAP_INDEX_PATH)
            );

            if (!response.ok) {
                throw new Error(
                    `Map index load failed: ${response.status}`
                );
            }
            ...
            if (
                !freshIndex ||
                !Array.isArray(freshIndex.maps) ||
                freshIndex.maps.length === 0
            ) {
                throw new Error('Fresh map index is empty.');
            }
