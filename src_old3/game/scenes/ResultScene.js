...
export class ResultScene extends Phaser.Scene {
    ...
    getPlayedMapDate() {
        ...
        /*
        mapId 시작 부분의 YYYY-MM-DD를 우선 사용한다.

        예:
        2026-07-16_KS11_1m_v5kospi
        → 2026-07-16
        */
        const match = String(mapId).match(
            /^(\d{4}-\d{2}-\d{2})(?:_|$)/
        );
        ...
        /*
        이전 형식의 맵을 위한 fallback.
        */
        return (
            this.mapData?.marketDate ||
            this.mapData?.date ||
            this.mapMeta?.marketDate ||
            this.mapMeta?.date ||
            'unknown date'
        );
    }
